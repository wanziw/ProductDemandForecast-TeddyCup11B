...
from utils import train_test_split, create_submission, compute_tweedie, compute_submission_rmse_WMAPE, compute_WMAPE, \
    label_encoding, inverse_label_encoding, delete_some_item, train_test_split_M2, add_prophet, create_final_submission, \
...
from error_analyze import delete_error_data_1, delete_error_data_2, final_delete_error_data_1, final_delete_error_data_2
...
valid_num = 45 # 43 44 45
is_M = 3  #是否滞后 0 2 3
# 误差分析
# raw_df = pd.read_csv("../中间数据/月度数据无编码_12.csv")
# sub_df = pd.read_csv(f"../提交数据/lgbm_submission_{is_M}.csv")
# final_delete_error_data_1(raw_df,sub_df)  # 筛除实际销量小的
# raw_df_2 = pd.read_csv("../中间数据/月度数据无编码_12_剔除误差.csv")
# final_delete_error_data_2(raw_df_2,sub_df)# 筛除实际销量大的 影响并不是很大
...
if valid_num < 43:
    ...
    submission.to_csv(f'../提交数据/lgbm_submission_{is_M}.csv', index=False)
    ...
    submission.to_csv(f'../提交数据/final_lgbm_submission_{valid_num}_{is_M}.csv', index=False)
